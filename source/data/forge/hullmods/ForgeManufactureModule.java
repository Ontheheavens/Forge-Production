...
public class ForgeManufactureModule extends BaseHullMod {
    ...
    public void applyEffectsBeforeShipCreation(ShipAPI.HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        if (stats.getVariant().getSMods().contains("forge_manufacture_module") ||
                stats.getVariant().getHullSpec().isBuiltInMod("forge_manufacture_module")) {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship != null)
        {
            ...
            boolean isInstalled = ship.getVariant().hasHullMod(ForgeTypes.MANUFACTURE);
            boolean hasCapacity = getShipCapacityWithCR(ship, hullSize) >= 1;

            int shipModifier = getShipCapacityMod(ship,hullSize, ForgeTypes.Conversion.MANUFACTURING);

            if (!isOperational || !isInstalled) {
                shipModifier = shipSizeEffect.get(hullSize);
            }

            ForgeHullmodsTooltip.addShipStateNote(tooltip, ship, hullSize, ForgeTypes.MANUFACTURE);
            ...
            float suppliesValue = ForgeSettings.SUPPLIES_PRODUCED * shipModifier;
            String supplies = String.valueOf((int)Math.floor(suppliesValue));
            if (suppliesValue < 1) {
                supplies = String.valueOf((int)Math.ceil(suppliesValue));
            }

            String metal = String.valueOf((int)Math.ceil(ForgeSettings.METAL_TO_MANUFACTURE * shipModifier));
            String transplutonics = String.valueOf((int)Math.ceil(ForgeSettings.TRANSPLUTONICS_TO_MANUFACTURE * shipModifier));
            ...
            if (!isOperational) {
                tooltip.addPara(firstLine, 4f, firstLineHighlights, supplies, metal, transplutonics);
            }
            ...
            ForgeHullmodsTooltip.addMachineryLine(tooltip, hullSize, ship, ForgeTypes.Conversion.MANUFACTURING,  ForgeTypes.MANUFACTURE);
            ...
            ForgeHullmodsTooltip.addSModLine(tooltip, ship, isForModSpec, ForgeTypes.MANUFACTURE) ;
